...
namespace prhlt {
    ...
    Algorithm_DP_Path_Finder::Algorithm_DP_Path_Finder(cv::Mat ex_image_mat, const Eigen::MatrixXd& ex_cost_matrix, cv::Rect ex_search_area ){
        ...
        LOG4CXX_INFO(this->logger,"<<Resizing to          : " << ex_search_area.x << " - " << ex_search_area.y << " : " << ex_search_area.width   << " - " << ex_search_area.height);
        this->axis_x = ex_search_area.y;
        this->axis_y = ex_search_area.x;
        ex_search_area.x = ex_search_area.x < 0 ? 0 : ex_search_area.x ; 
        ex_search_area.y = ex_search_area.y < 0 ? 0 : ex_search_area.y ; 
        ex_search_area.height = ex_search_area.y + ex_search_area.height > ex_cost_matrix.rows() ? ex_cost_matrix.rows()-ex_search_area.y  : ex_search_area.height; 
        ex_search_area.width = ex_search_area.x + ex_search_area.width > ex_cost_matrix.cols() ? ex_cost_matrix.cols()-ex_search_area.x  : ex_search_area.width; 
    ...
    vector<cv::Point2d> Algorithm_DP_Path_Finder::recover_best_path(){
        ...
        int min_index = 0;
        LOG4CXX_INFO(this->logger,"In row : " << min_index << " cost is : " << min_cost );
        ...
        //int i;
        //cin >> i;
        LOG4CXX_INFO(this->logger,"Choose row : " << min_index << " cost is : " << min_cost );
