...
namespace prhlt
{
    ...
    void Polyline_Extractor::run(vector<vector<cv::Point> > ex_regions, vector<vector<vector<cv::Point > > > ex_baselines, int ex_num_workers, float ex_dist_error, bool ex_enclosing_rect, int ex_up_dist, int ex_low_dist, int ex_horizontal_padding)
    {
        ...
        calculate_search_areas2(ex_regions, ex_baselines);
        ...
        for (int i = 0; i < ex_regions.size(); i++)
        {
            ...
            if (ex_baselines[i].size() > 0)
            {
                for (int j = 0; j < ex_baselines[i].size(); j++)
                {
                    ...
                    cv::Rect current_rect = cv::boundingRect(calculate_contour_from_baseline(ex_baselines[i][j]));
                    this->search_areas[i].push_back(current_rect); 
                    this->search_areas[i].push_back(current_rect); 
                }
            }
        }
    ...
    vector<cv::Point>  Polyline_Extractor::calculate_contour_from_baseline(const vector<cv::Point > &baseline){
        vector<cv::Point> res_contour; 
        for(int p=0; p < baseline.size(); p++){
            res_contour.insert(res_contour.begin(),cv::Point(baseline[p].x,baseline[p].y-this->up_dist));
            res_contour.push_back(cv::Point(baseline[p].x,baseline[p].y+this->low_dist));
        }
    ...
    void Polyline_Extractor::calculate_single_line_frontier_with_search_area(int i, int j)
    {
        ...
        else
        {
            if (j < this->search_areas[i].size() - 1)
            {
                ...
                vector<vector<cv::Point> > above_restrictions = expand_above_search_restrictions(i,j,0,0);
                vector<vector<cv::Point> > below_restrictions = expand_below_search_restrictions(i,j,0,0); 
                if(j % 2 == 0){

                    //path_finder_instance.set_search_limits(expand_other_search_restriction(i, j, 0, 0), expand_my_search_restriction(i, j, 0, -10));
                    vector<vector<cv::Point> > current_restrictions = expand_my_search_restriction(i, j, 0,-10); 
                    below_restrictions.insert( below_restrictions.end(), current_restrictions.begin(), current_restrictions.end());
                    path_finder_instance.set_search_limits(above_restrictions,below_restrictions);
                }
                else{
                    //path_finder_instance.set_search_limits(expand_my_search_restriction(i, j, 0, 0), expand_other_search_restriction(i, j, 0, 0));
                    vector<vector<cv::Point> > current_restrictions = expand_my_search_restriction(i, j, 0,10); 
                    above_restrictions.insert( above_restrictions.end(), current_restrictions.begin(), current_restrictions.end());
                    path_finder_instance.set_search_limits(above_restrictions,below_restrictions);
                }
    ...
    vector<vector<cv::Point> > Polyline_Extractor::expand_above_search_restrictions(int reg_index, int res_index, int x_offset, int y_offset){
        vector<vector<cv::Point> > res;
        vector<cv::Point> current_baseline = this->search_restrictions[reg_index][res_index]; 
            ...
            if(i!=res_index && ((res_index % 2 == 0 && res_index+1 != i ) || (res_index % 2 != 0 && res_index-1 != i)))  {
                for (int j = 0; j < this->search_restrictions[reg_index][i].size() - 1;j++){
                    ...
                    if(cv::clipLine(this->search_areas[reg_index][res_index], tmp[0], tmp[1])){
                        if(relative_possition_of_point_to_baseline(tmp[0],current_baseline)<0 && relative_possition_of_point_to_baseline(tmp[1],current_baseline) < 0) 
                            res.push_back(tmp);
                    }
                }
            }
    ...
    vector<vector<cv::Point> > Polyline_Extractor::expand_below_search_restrictions(int reg_index, int res_index, int x_offset, int y_offset){
        vector<vector<cv::Point> > res;
        vector<cv::Point> current_baseline = this->search_restrictions[reg_index][res_index]; 
        for (int i = 0; i < this->search_restrictions[reg_index].size();i++){
            if(i!=res_index && ((res_index % 2 == 0 && res_index+1 != i ) || (res_index % 2 != 0 && res_index-1 != i)))  {
    ...
    int Polyline_Extractor::relative_possition_of_point_to_baseline(cv::Point ex_point, const vector<cv::Point> & ex_baseline)
    {
        double abs_min = cv::norm(cv::Mat(ex_point),cv::Mat(ex_baseline[0]));
        int abs_min_index = 0; 

        for(int i = 1; i < ex_baseline.size(); i++){
            double current_min = cv::norm(cv::Mat(ex_point),cv::Mat(ex_baseline[i]));
        ...
        if(ex_point.y < ex_baseline[abs_min_index].y)
            pos = -1; 
        if(ex_point.y > ex_baseline[abs_min_index].y)
            pos = 1; 
    ...
    void Polyline_Extractor::calculate_line_frontiers()
    {
        ...
        void Polyline_Extractor::calculate_line_contour(int i, int j)
        {
            ...
            float tmp_distance = this->approx_dist_error;


            //LOG4CXX_ERROR(logger, "BEFORE SIMP " << tmp_contour.size());
            if(tmp_contour.size() > 0 && upper_frontier.size() > 0 && lower_frontier.size() > 0 ){
                if(this->approx_dist_error == -1){
                    vector <cv::Point> hull; 
                    cv::convexHull(tmp_contour,hull); 
                    tmp_distance=arcLength(hull,true)*0.005; 
                }
